...
onMounted(() => {
    ...
    viewType.value = store.togglers.find(e => e.name === 'viewType_5').value
})
...
watch(store.togglers, (val) => {
    viewType.value = val.find(e => e.name === 'viewType_5').value
})
...
POSMLayout
    ...
    TotalProgressbar(isMillion :data="totalData" :ignore="[]" :viewType="viewType")
        template(#legend)
